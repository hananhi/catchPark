  ...
  function getLocation(){
    ...
    const handleWazeLinkClick = (e, lo) => {
        ...
        // Fallback to Waze website
            ...
            window.open(`https://www.waze.com/ul?ll=${encodeURIComponent(lo.location?.locationp?.coordinates[1])},${encodeURIComponent(lo.location?.locationp?.coordinates[0])}&navigate=yes&zoom=17&from=${encodeURIComponent(userLocation.lat)},${encodeURIComponent(userLocation.lng)}`, '_blank');
...
/*
       ...
       {userLocation && (
        <>
          <p>
            Live Location - Latitude: {userLocation.lat}, Longitude: {userLocation.lng}
          </p>
          {userAddress && <p>Address: {userAddress}</p>}
          <button onClick={()=>reportLocation(userLocation,userAddress)}>report empty park</button><br/>
          <button onClick={()=>getLocation()}>get locations</button>

          <button onClick={()=>getClosePark(userLocation,dataArray)}>closet location</button>
          {updatedLocation.map((lo, index) => (
  <div key={index}>
    <div>{lo.location.address}</div>
    <div>{lo.duration}</div>
  
    <a
        href={`https://www.waze.com/ul?ll=${encodeURIComponent(lo.location?.locationp?.coordinates[1])},${encodeURIComponent(lo.location?.locationp?.coordinates[0])}&navigate=yes&zoom=17&from=${encodeURIComponent(userLocation.lat)},${encodeURIComponent(userLocation.lng)}`}
        target="_blank"
...
<button onClick={(e)=>handleDeleteLocaton(lo)}>get the park </button>
  </div>
))}
        </>
      )}
